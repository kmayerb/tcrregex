...
pb_cdrs = repertoire_db.generate_pbr_cdr() # This replaces: from tcrregex.cdr3s_human import pb_cdrs
...
class TCRrep:
    ...
    paired_tcrregex : ndarray
        most recent output of :py:meth:`tcrregex.repertoire.TCRrep.compute_paired_tcrregex`
    paired_tcrregex_weights : dictionary
        CDR weights used to generate the most recent output of
        TCRrep.compute_paired_tcrregex`
    ...
    def __init__(self,
        ...
        self.paired_tcrregex = None
        self.paired_tcrregex_weights = None
        self.meta_cols = None
    ...
    def deduplicate(self):
        ...
        if np.sum(self.cell_df['count']) != np.sum(self.clone_df['count']):
            ...
            warnings.warn(f"Not all cells/sequences could be grouped into clones. {n_cells_lost} of {n_cell} were not captured. This occurs when any of the values in the index columns are null or missing for a given sequence. To see entries with missing values use: tcrregex.repertoire.TCRrep.show_incomplete()\n")
    ...
    def infer_cdrs_from_v_gene(self, chain, imgt_aligned = False):
        ...
        originally: from tcrregex.cdr3s_human import pb_cdrs
    ...
    def infer_olga_aa_cdr3_pgens(self,
        ...
        (Sethna et al. 2018) updated to python 3 for use with tcrregex.
        ...
        Notes
        -----
        tcrregex2 authors UPDATED THE FOLLOWING CODE TO PYTHON 3
    ...
    def archive(self, 
        ...
        Notes
        -----
        See :py:meth:`tcrregex.repertoire.rebuild`: for reubilding a TCRrep instance from 
    ...
    def rebuild(self,  dest_tar_name = "default_archive.tar.gz", verbose = True ):
        ...
        Example
        -------
        Shows :py:meth:`tcrregex.repertoire.archive` and :py:meth:`tcrregex.repertoire.rebuild` 
        used together.
        ...
        Notes
        -----
        See :py:meth:`tcrregex.repertoire.archive` for creating TCRrep archive file.
        """
    ...
    def tcrregex2(self, 
        ...
        Notes
        -----
        tcrregex2 is a method to help new-users run tcrregex2 with sensible defaults.

        Distance metrics are highly customizable. 
        Consult the `docs <https://tcrregex2.readthedocs.io>`_ for more information.
        
        To compute Dash et al. 2017 style tcrregexance, instead of tcrregex2,
        use commands:
        TCRrep._tcrregex_legacy_method_alpha_beta()
        TCRrep._tcrregex_legacy_method_beta()
        TCRrep._tcrregex_legacy_method_alpha()
        TCRrep._tcrregex_legacy_method_gamma_delta()
        TCRrep._tcrregex_legacy_method_gamma()
        TCRrep._tcrregex_legacy_method_delta()
        """
        ...
        if processes is None:
            ...
            sys.stdout.write(f"\tThe `processes` arg of TCRrep.tcrregex2() can be set manually\n")
        ...
        sys.stdout.write("Calculating composite tcrregexance measures:\n")
        rep_weights = {f"{k}_pw":w for k,w in weights.items() if k in self.cell_df.columns and k.endswith('_aa')}

        self.compute_paired_tcrregex( chains=self.chains, store_result=False, replacement_weights = rep_weights)
        for chain in self.chains:
            if verbose: sys.stdout.write(f"\tSingle chain pairwise tcrregexances are in attribute : TCRrep.pw_{chain}\n")
        if verbose: sys.stdout.write(f"\tCombined pairwise tcrregexances are in attribute     : TCRrep.pw_tcrregex\n")
        if verbose: sys.stdout.write(f"\tCDR specific tcrregexances are in attributes, e.g.,  : TCRrep.cdr3_{chain[0]}_aa_pw\n")
        ...
        if verbose: sys.stdout.write(f"TCRrep.tcrregex2() COMPLETED SUCCESSFULLY, see the docs for Analysis steps!\n")
    ...
    def compute_pairwise_all(self,
        ...
        metric : string
            'nw', 'hamming', or 'custom' (or if legacy tcrregex is to be calculated,
            "tcrregex_cdr3", "tcrregex_cdr1", "tcrregex_cdr2",
            "tcrregex_cdr2.5", "tcrregex_pmhc" can be supplied. WARNING:
        ...
        if metric in ["tcrregex_cdr3", "tcrregex_cdr1", "tcrregex_cdr2",
                      "tcrregex_cdr2.5", "tcrregex_pmhc"]:
    ...
    def compute_paired_tcrregex(self,
                               chains = ['alpha', 'beta'],
                               replacement_weights = None,
                               store_result = False):
        """
        Computes tcrregexance metric combining distances metrics across multiple
        ...
        store_result : boolean
            ...
            :py:attr:`TCRrep.stored_tcrregex`

        Returns
        -------
        r : dictionary
            a dictionary with keys paired_tcrregex points to a 2D
            tcrregex np.ndarray and paired_tcrregex_weights pointing to
        ...
        `TCRrep.paired_tcrregex` and
        `TCRrep.paired_tcrregex_weights`
        and stores r to
        `TCRrep.stored_tcrregex`

        In addition it returns a dictionary with keys `paired_tcrregex` 2D
        tcrregex np.array and `paired_tcrregex_weights`
        a dictionary of regions and relative weights:

        {'paired_tcrregex': array([[ 0., 76., 80.,..., 89., 89., 87.],
                                [ 76., 0., 60., ..., 81., 75., 43.],
                                [ 80., 60., 0., ..., 59., 81., 77.],
                                ...,
                                [ 89., 81., 59.,  ..., 0., 60., 58.],
                                [ 89., 75., 81.,   ..., 60., 0., 40.],
                                [ 87., 43., 77., ..., 58., 40., 0.]]),
        'paired_tcrregex_weights': {'cdr1_a_aa_pw': 1,
                                   'cdr1_b_aa_pw': 2,
                                   'cdr2_a_aa_pw': 1,
                                   'cdr2_b_aa_pw': 2,
                                   'cdr3_a_aa_pw': 2,
        ...
        if 'alpha' in chains:
            tcrregex = np.zeros(self.cdr3_a_aa_pw.shape)
            for k in alpha_keys:
                try:
                    tcrregex = self.__dict__[k]*weights[k] + tcrregex
                except KeyError:
                    warnings.warn("tcrregex was calculated without: '{}' because pairwise distances haven't been computed for this region:".format(k))
            self.pw_alpha = tcrregex
        if 'beta' in chains:
            tcrregex = np.zeros(self.cdr3_b_aa_pw.shape)
            for k in beta_keys:
                try:
                    tcrregex = self.__dict__[k]*weights[k] + tcrregex
                except KeyError:
                    warnings.warn("tcrregex was calculated without: '{}' because pairwise distances haven't been computed for this region:".format(k))
            self.pw_beta = tcrregex
        if 'gamma' in chains:
            tcrregex = np.zeros(self.cdr3_g_aa_pw.shape)
            for k in gamma_keys:
                try:
                    tcrregex = self.__dict__[k]*weights[k] + tcrregex
                except KeyError:
                    warnings.warn("tcrregex was calculated without: '{}' because pairwise distances haven't been computed for this region:".format(k))
            self.pw_gamma = tcrregex
        if 'delta' in chains:
            tcrregex = np.zeros(self.cdr3_d_aa_pw.shape)
            for k in delta_keys:
                try:
                    tcrregex = self.__dict__[k]*weights[k] + tcrregex
                except KeyError:
                    warnings.warn("tcrregex was calculated without: '{}' because pairwise distances haven't been computed for this region:".format(k))
            self.pw_delta = tcrregex
        
        # For combined chain tcrregex, restults in TCRrep.paired_tcrregex and TCRrep.pw_tcrregex
        full_keys = []
        if 'alpha' in chains:
        ...
        # initialize tcrregex matrix size
        for k in full_keys:
            try:
                tcrregex = np.zeros(self.__dict__[k].shape)
                break
            except KeyError:
                pass
        ...
        # keep 'paired_tcrregex' to avoid breaking tests
        self.paired_tcrregex = tcrregex
        self.pw_tcrregex = tcrregex
        self.paired_tcrregex_weights = {k:weights[k] for k in full_keys}
        
        # Typically we don't want to store different tcrregexance in the same repertoire, but
        r = {'paired_tcrregex' : tcrregex,
            'paired_tcrregex_weights' : {k:weights[k] for k in full_keys}}
        if store_result:
            self.stored_tcrregex.append(r)
        return(r)
    ...
    def simple_cluster_index(   self,
        ...
        if pw_distances is None:
            pw_distances = self.pw_tcrregex
        
        compressed_dmat = scipy.spatial.distance.squareform(self.paired_tcrregex, force = "vector")
    ...
    def generate_cluster_index(self, t = 75, criterion = "distance", method =  "complete", append_counts = False):
        ...
        assert len(cluster_index) == self.paired_tcrregex.shape[0]
    ...
    def _tcrregex_legacy_method_alpha_beta(self, processes = 1):
        """
        Runs the legacy tcrregex pairwise comparison
        ...
        Notes
        -----
        # CALCULATE tcrregex distance metric. Here we show all the manual steps to
        # implement the original Dash et al. tcrregex approach.

        # To do this we calculate distance for each CDR separately, and
        # we use the metric "tcrregex_cdr3" for the cdr3 and "tcrregex_cdr1"
        # everywhere else
        """
        ...
        self.compute_pairwise_all(chain = "alpha",                        # <11
                                 metric = 'tcrregex_cdr3',
                                 compute_specific_region = 'cdr3_a_aa',
                                 processes = processes)
        self.compute_pairwise_all(chain = "alpha",                        # 11
                                 metric = "tcrregex_cdr1",
        ...
        self.compute_pairwise_all(chain = "beta",                         # 12
                                 metric = 'tcrregex_cdr3',
                                 #user_function = tcrregex_metric_align_cdr3s_false,
        ...
        distA = self.compute_paired_tcrregex(replacement_weights= {'cdr3_a_aa_pw': 1,
                                                                'cdr2_a_aa_pw': 1,
                                                                'cdr1_a_aa_pw': 1,
                                                                'pmhc_a_aa_pw': 1,
                                                                'cdr3_b_aa_pw': 0,
                                                                ...
                                                                'cdr1_b_aa_pw': 0,
                                                                'pmhc_b_aa_pw': 0},
                                                                 chains = ["alpha", "beta"])['paired_tcrregex'].copy()

        distB = self.compute_paired_tcrregex(replacement_weights= {'cdr3_a_aa_pw': 0,
                                                                'cdr2_a_aa_pw': 0,
                                                                'cdr1_a_aa_pw': 0,
                                                                'pmhc_a_aa_pw': 0,
                                                                'cdr3_b_aa_pw': 1,
        ...
        # Calling tr.compute_paired_tcrregex() computs the
        # the final paired chain TCR-distance which is stored as
        # tr.paired_tcrregex, which we confirm is simply the sum of distA and distB
        self.compute_paired_tcrregex(chains = self.chains)
        assert np.all(((distA + distB) - self.paired_tcrregex) == 0)
        self.pw_tcrregex = self.paired_tcrregex
        self.pw_alpha = distA
        self.pw_beta  = distB

        # tr.paired_tcrregex and distA, distB are np arrays, but we will want to work with as a pandas DataFrames
    ...
    def _tcrregex_legacy_method_alpha(self, processes = 1):
        """
        Runs the legacy tcrregex pairwise comparison
        ...
        distA = self.compute_paired_tcrregex(replacement_weights= {'cdr3_a_aa_pw': 1,
                                                                ...
                                                                'cdr1_a_aa_pw': 1,
                                                                'pmhc_a_aa_pw': 1},
                                                                chains = ["alpha"])['paired_tcrregex'].copy()
                                                                        # Calling tr.compute_paired_tcrregex() computs the
        # the final paired chain TCR-distance which is stored as
        # tr.paired_tcrregex, which we confirm is simply the sum of distA and distB
        self.compute_paired_tcrregex(chains = self.chains)
        assert np.all((distA - self.paired_tcrregex) == 0)

        # tr.paired_tcrregex and distA, distB are np arrays, but we will want to work with as a pandas DataFrames
        self.dist_a = pd.DataFrame(distA, index = self.clone_df.clone_id, columns = self.clone_df.clone_id)
        self.pw_alpha = distA
        
    def _tcrregex_legacy_method_beta(self, processes = 1):
        """
        Runs the legacy tcrregex pairwise comparison
        ...
        distB = self.compute_paired_tcrregex(replacement_weights= {
                                                                'cdr3_b_aa_pw': 1,
                                                                'cdr2_b_aa_pw': 1,
                                                                'cdr1_b_aa_pw': 1,
                                                                'pmhc_b_aa_pw': 1},
                                                                chains = ["beta"])['paired_tcrregex'].copy()

        self.compute_paired_tcrregex(chains = self.chains)
        assert np.all(((distB) - self.paired_tcrregex) == 0)
    ...
    def _tcrregex_legacy_method_gamma_delta(self, processes = 1):
        """
        Runs the legacy tcrregex pairwise comparison gamma/delta
        ...
        distA = self.compute_paired_tcrregex(replacement_weights= {'cdr3_g_aa_pw': 1,
                                                                'cdr2_g_aa_pw': 1,
                                                                'cdr1_g_aa_pw': 1,
                                                                'pmhc_g_aa_pw': 1,
                                                                'cdr3_d_aa_pw': 0,
                                                                ...
                                                                'cdr1_d_aa_pw': 0,
                                                                'pmhc_d_aa_pw': 0},
                                                                chains = ["gamma", "delta"])['paired_tcrregex'].copy()

        distB = self.compute_paired_tcrregex(replacement_weights= {'cdr3_g_aa_pw': 0,
                                                                'cdr2_g_aa_pw': 0,
                                                                'cdr1_g_aa_pw': 0,
                                                                'pmhc_g_aa_pw': 0,
                                                                'cdr3_d_aa_pw': 1,
        ...
        # the final paired chain TCR-distance which is stored as
        # tr.paired_tcrregex, which we confirm is simply the sum of distA and distB
        self.compute_paired_tcrregex(chains = ["gamma", "delta"])
        assert np.all(((distA + distB) - self.paired_tcrregex) == 0)
    ...
    def drop_stored_results(self):
        ...
        the most recent self.paired_tcrregex

        """
        self.stored_tcrregex = None
    ...
    def reduce_file_size(self, attributes = None, data_type = 'int16', verbose = True):
        ...
        data_type : str
            ...
            tcrregexances but save considerable space in memory.
...
def _compute_pairwise(  sequences, 
    ...
    metric : string
        Options include 'nw', hamming', 'hamming_aligned','tcrregex_cdr3', "tcrregex_cdr1", 'tcrregex_cdr2', 'tcrregex_cdr2.5', 'tcrregex_pmhc'
    processes : int
    ...
    **kwargs : keyword arguments passed to metric function
    ...
    elif metric == 'tcrregex_cdr3':
        metric_func = pairwise.tcrregex_cdr3_metric
    elif metric in ['tcrregex_cdr1', 'tcrregex_cdr2', 'tcrregex_cdr2.5', 'tcrregex_pmhc']:
        metric_func = pairwise.tcrregex_cdr1_metric
