...
import pandas as pd 
import numpy as np
from tcrregex import repertoire_db
import warnings


def mixcr_to_tcrregex2(chain:str, 
                      organism:str, 
                      seqs_fn:str = None, 
                      clones_fn:str = None):
    """
    Converts .clns.txt or .result.txt outputs from mixcr to tcrregex2
    formatted input. 
    ...
    .. code-block:: python
        ...
        from tcrregex import mixcr
        
        clones_fn = os.path.join('tcrregex',
                                'test_files_compact',
                                'SRR5130260.1.test.fastq.output.clns.txt')

        df = mixcr.mixcr_to_tcrregex2(chain = "delta", 
                                    organism = "human", 
                                    clones_fn = clones_fn)
...
def remove_entries_with_invalid_vgene(df, chain:str,organism:str):
    ...
    ----------
    df : pd.DataFrame
        DataFrame produced by mixcr.mixcr_to_tcrregex2
    chain : str
        'alpha', 'beta', 'gamma', or 'delta'
    organism : str
        'human' or 'mouse"
...
def _change_TRAVDV_to_TRAVdashDV(s:str):
    """
    Reconciles mixcr name like TRAV29/DV5*01 to tcrregex2 name TRAV29DV5*01
    ...
    Notes
    -----
    This reconciles such gene names to match the tcrregex2 reference db.
...
def _validate_gene_names(series, chain:str, organism:str):
    ...
    valid : pd.Series
        series of booleans where True means name is valid and in tcrregex database
