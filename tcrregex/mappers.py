...
reformatting it to match tcrregex2 input requirements.
...
tcrregex_to_tcrregex2_mapping = OrderedDict([('id', 'id'),
                                           ('epitope', 'epitope'),
                                           ('subject', 'subject'),
                                           ('cdr3a'  , 'cdr3_a_aa'),
                                           ('cdr3b'  , 'cdr3_b_aa'),
...
vdjdb_to_tcrregex2_mapping_TRA = OrderedDict([('complex.id', 'complex_id'),
                                        ('Gene', 'gene'),
                                        ('CDR3', 'cdr3_a_aa'),
                                        ('V', 'v_a_gene'),
                                        ('J', 'j_a_gene'),
...
vdjdb_to_tcrregex2_mapping_TRB = OrderedDict([('complex.id', 'complex_id'),
                                        ('Gene', 'gene'),
                                        ('CDR3', 'cdr3_b_aa'),
                                        ('V', 'v_b_gene'),
                                        ('J', 'j_b_gene'),
...
tcrregex_clone_df_to_tcrregex2_mapping = OrderedDict([ ('clone_id'  , 'clone_id' ),
                                                     ('subject'   , 'subject'  ),
                                                     ('cdr3a'     , 'cdr3_a_aa'),
                                                     ('cdr3b'     , 'cdr3_b_aa'),
                                                     ('clone_size', 'count'    ),
...
tcrregex2_to_tcrregex_clone_df_mapping_gd_red = OrderedDict([('clone_id', 'clone_id'),
                                                        ('subject', 'subject'),
                                                        ('cdr3_g_aa', 'cdr3a'),
                                                        ('cdr3_d_aa', 'cdr3b'),
                                                        ('count', 'clone_size'),
...
tcrregex2_to_tcrregex_clone_df_mapping_gd = OrderedDict([('clone_id', 'clone_id'),
                                                        ('subject', 'subject'),
                                                        ('cdr3_g_aa', 'cdr3a'),
                                                        ('cdr3_d_aa', 'cdr3b'),
                                                        ('count', 'clone_size'),
...
tcrregex2_to_tcrregex_clone_df_mapping = OrderedDict([('clone_id', 'clone_id'),
                                                     ('subject', 'subject'),
                                                     ('cdr3_a_aa', 'cdr3a'),
                                                     ('cdr3_b_aa', 'cdr3b'),
                                                     ('count', 'clone_size'),
...
tcrregex2_to_tcrregex_clone_df_mapping_a = OrderedDict([('clone_id', 'clone_id'),
                                                     ('subject', 'subject'),
                                                     ('cdr3_a_aa', 'cdr3a'),
                                                     ('count', 'clone_size'),
                                                     ('epitope', 'epitope'),
...
tcrregex2_to_tcrregex_clone_df_mapping_b = OrderedDict([('clone_id', 'clone_id'),
                                                     ('subject', 'subject'),
                                                     ('cdr3_b_aa', 'cdr3b'),
                                                     ('count', 'clone_size'),
                                                     ('epitope', 'epitope'),
...
# Note that original tcrregex had no concept of gamma and delta syntax g->a , d->b 


tcrregex2_to_tcrregex_clone_df_mapping_g = OrderedDict([('clone_id', 'clone_id'),
                                                        ('subject', 'subject'),
                                                        ('cdr3_g_aa', 'cdr3a'),
                                                        ('count', 'clone_size'),
                                                        ('epitope', 'epitope'),
...
tcrregex2_to_tcrregex_clone_df_mapping_d = OrderedDict([('clone_id', 'clone_id'),
                                                        ('subject', 'subject'),
                                                        ('cdr3_d_aa', 'cdr3b'),
                                                        ('count', 'clone_size'),
                                                        ('epitope', 'epitope'),
...
def vdjdb_to_tcrregex2(pd_df):
    """
    reformat .tsv downloaded from vdjdb (July 2019) to input format of tcrregex2 input format
    ...
    for index, row in pd_df.iterrows():
        ...
        if gene == "TRA":
            row = row.rename(vdjdb_to_tcrregex2_mapping_TRA)
        elif gene == "TRB":
            row = row.rename(vdjdb_to_tcrregex2_mapping_TRB)
    ...
    tcrregex2_formatted_pd_df = pd.DataFrame.from_dict(l_out)
    return(tcrregex2_formatted_pd_df)



def multistudy_vdjdb_to_tcrregex2(pd_df):
    """

    map vdjdb data from multiple studies to one tcrregex2 formatted pd.dataframe
    ...
    tcr_formatted_dfs_merged : Pandas DataFrame
        with all paired data from input, single combined data frame tcrregex2 mapped
    ...
    # tcrregex format each study specific dataframe
    tcr_formatted_dfs_split_by_study = {study: None for study in studies}
    for study in list(dfs_split_by_study.keys()):
        tcr_formatted_dfs_split_by_study[study] = vdjdb_to_tcrregex2(pd_df = dfs_split_by_study[study])
...
def map_genes_to_first_alleles(genes, organism):
    ...
    from collections import OrderedDict
    pd_df = pd.read_csv('tcrregex/db/alphabeta_db.tsv', sep = '\t')
    ...
    # Load second file
    pd_df = pd.read_csv('tcrregex/db/gammadelta_db.tsv', sep = '\t')
    g = pd_df.id.str.split("*")
    gene = [x[0] for x in g]
    X = pd.DataFrame({'gene' : gene, 'organism' : pd_df.organism.copy(), 'id' : pd_df.id.copy()})
    X = X.groupby(['gene', 'organism']).first().reset_index()
...
def populate_legacy_fields(df, chains =['alpha', 'beta']):
    ...
    'va_countreps' and 'va_gene' for use of tcrregex.subset.TCRsubset()
    """
    if 'alpha' in chains:
