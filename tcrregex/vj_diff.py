...
def vj_surprise(member_df, tsamp, chain='beta', count_col=None,
    ...
    if v_col is None or j_col is None:
        tcrregex2_genes =  { 'alpha' :  ['v_a_gene', 'j_a_gene'],
                            'beta'  :  ['v_b_gene', 'j_b_gene'],
                            'gamma'  : ['v_g_gene', 'j_g_gene'],
                            'delta'  : ['v_d_gene', 'j_d_gene']}
        v_col, j_col = tcrregex2_genes[chain]
...
def _vj_stats_validate_inputs(chain,freq_type, df, ts, cols):
    ...
    are likely to work with tcrregex2 
    ...
    if chain not in tcrregex2_genes.keys():
    ...
    all_tcrregex2_genes = np.concatenate(list(tcrregex2_genes.values()))
    if not np.sum([col in all_tcrregex2_genes for col in df.columns ]) >= 1:
        raise KeyError(f'_vj_stats argument < df > DataFrame must contain 1 or more columns from {all_tcrregex2_genes}')
    valid = True
    return valid
