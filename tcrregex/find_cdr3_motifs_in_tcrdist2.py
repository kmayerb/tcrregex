...
def find_cdr3_motif(
            clones_file             = "/Users/kmayerbl/PycharmProjects/tcrregex2/tcrregex2to3/mouse_pairseqs_v1_parsed_seqs_probs_mq20_clones.tsv",
            organism                = "mouse",
            epitopes                = ['PA'],
            chains                  = ['A','B'],
            ...
            all_genes                = None):
    """
    find_cdr3_motif preserves the behavior of the motif finder in tcrregex v1.
    ...
    if constant_seed:
        random.seed(1)
    assert big, "big must be True to Match tcrregex v1 behavior"
    assert unmask, "unmask must be True to match tcrregex v1 behavior"
    assert use_groups, 'use_groups must be True to match tcrregex va behavior'
